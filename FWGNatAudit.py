...
def ResultHeaders():
	...
	wsResult.Cells(1,3).Value  = "NAT Pool 1"
	wsResult.Cells(1,4).Value  = "NAT Pool 2"
	wsResult.Cells(1,5).Value  = "NAT Pool 3"
	wsResult.Cells(1,6).Value  = "NAT Pool 4"
	wsResult.Cells(1,7).Value  = "NAT Pool 5"
...
def AnalyzeResults(strOutputList):
	...
	for strLine in strOutputList:
		...
		if "ip nat pool " in strLine:
			strLineTokens = strLine.split(" ")
			if len(strLineTokens) > 8:
				wsResult.Cells(iOutLineNum,2+iIntNum).Value = strLineTokens[4] + strLineTokens[7]
				iIntNum += 1
...
def GetResults(strHostname,strCmd):
	try:
		...
		while not objShell.recv_ready():
			time.sleep(1)
		strResp = objShell.recv(9999) # Clear the buffer
		objShell.send(strCmd+"\n")
		print ("sent {0} to {1}".format(strCmd,strHostname))
		while not objShell.recv_ready():
			time.sleep(1)
		strResp = objShell.recv(9999)
