...
def CollectApplianceData (dictTemp):
	...
	if dictTemp["STATUS"]=="Offline" and bNotify:
		if int(dictTemp["HEARTBEATS_MISSED"]) < 700 and int(dictTemp["HEARTBEATS_MISSED"]) > 0:
			SendNotification("Scanner {} has been Offline since {} GMT".format(dictTemp["NAME"],dictOut["LastConn"]))
	else:
		if dictTemp["ML_VERSION"]["@updated"] == "no" and dictOut["MLVer"] != "" and bNotify:
			SendNotification("Scanner {} is at version {}, latest is {}".format(dictTemp["NAME"],dictOut["MLVer"],dictTemp["ML_LATEST"]))
		if dictTemp["VULNSIGS_VERSION"]["@updated"] == "no" and dictOut["VULNVER"] != "" and bNotify:
			SendNotification("Signatures on {} are at version {}, latest is {}".format(dictTemp["NAME"],dictOut["VULNVER"],dictTemp["VULNSIGS_LATEST"]))
...
def UpdateDB (dictAppliance):
	...
	if not ValidReturn(lstReturn):
		...
		strSQL = ("INSERT INTO tblAppliances (iApplianceID,vcUUID,vcName,vcState,vcModel,vcType,vcSerialNum,vcIPAddr1,vcGW1,iIPaddr1,iGW1,vcInt2State,vcIPAddr2,vcGW2,iIPAddr2,iGW2,"
					"vcDNS1_1,vcDNS1_2,vcDNS2_1,vcDNS2_2,vcProxyState,dtLastAPIUpdate,iMissedHB,dtLastConnected,vcLatestML,vcLatestVuln,vcSoftVer,dtLatestUpdate,vcMLVer,vcVulnVer) "
					"VALUES({},'{}','{}','{}','{}','{}','{}','{}','{}',{},{},'{}','{}','{}',{},{},'{}','{}','{}','{}','{}',now(),{},{},'{}','{}','{}',{},'{}','{}');".format(
						...
						dictAppliance["MissedHB"],dictAppliance["LastConn"],dictAppliance["ML_LATEST"],dictAppliance["VulnLatest"],dictAppliance["SoftVer"],dictAppliance["LastUpdate"],dictAppliance["MLVer"],dictAppliance["VULNVER"])
				  )
	elif lstReturn[0] == 1:
		LogEntry ("Appliance {} exists, need to update {}".format(dictAppliance["ID"],dictAppliance["name"]))
		strSQL = ("UPDATE tblAppliances SET vcUUID = '{}', vcName = '{}', vcState = '{}', vcModel = '{}', vcType = '{}', vcSerialNum = '{}', vcIPAddr1 = '{}', vcGW1 = '{}', iIPaddr1 = {}, "
					...
					" vcSoftVer = '{}',dtLatestUpdate = {},vcMLVer = '{}',vcVulnVer = '{}' WHERE iApplianceID = {};".format(
						...
						dictAppliance["MLVer"],dictAppliance["VULNVER"],dictAppliance["ID"]
						)
				)
	else:
		LogEntry ("Something is horrible wrong, there are {} appliance with ID of {}".format(lstReturn[0],dictAppliance["ID"]))
		return "Abort!!!"
	lstReturn = SQLQuery (strSQL,dbConn)
	if not ValidReturn(lstReturn):
		LogEntry ("Unexpected: {}".format(lstReturn))
...
LogEntry ("VALIDATE: Total Number of vulnerabitlities downloaded {}; Total number of vulnerabitlities updated in the database {}".format(iNumRows,iCountChange))
if iNumRows != iCountChange:
  ...
  SendNotification("{} has completed processing on {}, validation checks are good. Processed {} scanner appliances.".format(strScriptName,strScriptHost,iNumRows))
